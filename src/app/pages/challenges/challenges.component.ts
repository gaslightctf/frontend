...
export class ChallengesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.ctfStartSubscription = this.dataService
      .getCTFStart()
      .subscribe((ctfStart) => {
        this.ctfStart = ctfStart;
      });
    this.hasCTFStartedSubscription = this.dataService.hasCTFStarted.subscribe(
      (hasCTFStarted) => {
    ...
    this.challengeDetailCategoriesSubscription = combineLatest([
      ...
      .pipe(
        map((data) => {
          ...
          if (filterCategory != "") {
            filteredChallengeDetailCategories =
              filteredChallengeDetailCategories.filter(
